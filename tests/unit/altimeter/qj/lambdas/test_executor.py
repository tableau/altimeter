...
@moto.mock_sqs
def test_enqueue_queries_with_jitter():
    max_jitter_sec = 5
...
@moto.mock_sqs
def test_enqueue_queries_without_jitter():
    sqs_client = boto3.client("sqs", region_name="us-west-2")
    queue_url = sqs_client.create_queue(QueueName="test")["QueueUrl"]
    job_1 = Job(
        name="fooboo",
    ...
    jobs: List[Job] = [job_1, job_2]
    enqueue_queries(
        jobs=jobs, queue_url=queue_url, execution_hash="1234", region="us-west-2",
    )
    msgs = []
    resp = sqs_client.receive_message(QueueUrl=queue_url)
    for raw_msg in resp["Messages"]:
