...
def aws2neptune(scan_id: str, config: Config, muxer: AWSScanMuxer, load_neptune: bool) -> AWS2NResult:
    ...
    rdf_path = artifact_writer.write_graph_set(
        name="master", graph_set=graph_set, compression=GZIP)
    graph_metadata = None
    if load_neptune:
        ...
        neptune_client = AltimeterNeptuneClient(
            max_age_min=1440, neptune_endpoint=endpoint)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    result = aws2n(scan_id=scan_id, config=config,
                   muxer=muxer, load_neptune=False)
